...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    /**
     * TODO: Initialize the state ekf_.x_ with the first measurement.dodd
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      float del_x = rho * cos(phi);
      float del_y = rho * sin(phi);
      float vx = rho_dot * cos(phi);
      float vy = rho_dot * sin(phi);

      ekf_.x_ << del_x, del_y, vx, vy;
    }
  ...
  float dt_4_by_4 = dt_4 / 4;  // dt^4/4
  float dt_3_by_2 = dt_3 / 2; // dt^3/2
